...
import { CartItemWithProduct as CartItem } from '@/types/type';
...
function useRandomProducts(products: any[], count: number) {
  return useMemo(() => {
...
export default function CartPage() {
  ...
  const randomProducts = useRandomProducts(products || [], 4);

  const amount = useMemo(() => {
    if (!product) return 0;
    return product
      .map((item: CartItem) => Number(item.amount))
  ...
  const handleOrder = () => {
    if (!product) return;
    addOrder({
      items: product.map((cart: CartItem) => ({
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 lg:py-16">
        ...
        <div className="flex gap-10 flex-col lg:flex-row">
          ...
          <div className="flex-1 space-y-4">
            {product ? (
              product.length > 0 ? (
                product.map((item: CartItem, idx: number) => (
...
const Item = (props: CartItem & { deleteCart: (id: number) => void }) => {
  ...
  const handleItemUpdate = () => {
    if (!quantity) return;
    updateCart({ id: props.id, quantity, amount: props.price * quantity });
  };
