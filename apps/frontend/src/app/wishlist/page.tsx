...
import { useWishlistItems } from '@/hooks/queries/useQuery';

export default function Wishlist() {
  const { data: products } = useWishlistItems();

  return (
    <main className="min-h-screen pt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 lg:py-16">
        <h1 className="text-2xl font-bold mb-8">Wishlist ({products?.length ?? 0})</h1>
        <div className="space-y-3">
          {products ? (
            products.length > 0 ? (
              products.map((item: WishlistItem, idx: number) => <Item key={idx} {...item} />)
            ) : (
              <EmptyBox />
            )
          ) : (
