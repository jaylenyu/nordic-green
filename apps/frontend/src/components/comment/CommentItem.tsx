...
export default function CommentItem({
  ...
  const { mutate: upsertComment } = useUpsertComment(productId);
  const { mutate: deleteComment } = useDeleteComment(productId);
  ...
  const handleSave = () => {
    ...
    upsertComment(
      {
        orderItemId: Number(item.orderItemIds),
        rate,
  ...
  const handleDeleteComment = () => {
    ...
    deleteComment(Number(item.id), {
      onSuccess: () => toast.success('댓글이 삭제되었습니다.'),
      onError: () => toast.error('댓글 삭제 실패. 다시 시도해주세요.'),
    });
  };
